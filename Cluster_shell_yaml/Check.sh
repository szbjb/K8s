...
# for var_01 in  $(awk -F  "="   '{print  $2}' /root/K8s/ip_2.txt|xargs)
#         do 
#        var_01_b=$(ansible $var_01 -m  shell -a "docker version|grep  Version|head  -n 1"|grep -A 1 ${var_01} |grep -v rc=  |grep  -o  "18.09.7-ce")
#        ansible  ${var_01} -m  shell -a "docker version"|grep  "Version" && {
# [  $var_01_b = 18.09.7-ce ]  || echo  "主机${var_01}docker版本不是18.09.7-ce，请重新安装18.09.7-ce版本"  >>/root/K8s/Cluster_shell_yaml/err_check.log
# }
...
# for var_01 in  $(awk -F  "="   '{print  $2}' /root/K8s/ip_2.txt|xargs)
#         do 
#        ansible ${var_01} -m shell   -a  "ss -lntup| egrep  \"8080|42344|6443|\"" >/dev/null  && {
#        echo "主机${var_01}存在以下端口占用，请停止相关服务接触端口占用"
#        echo "主机${var_01}存在以下端口占用，请停止相关服务接触端口占用"  >>/root/K8s/Cluster_shell_yaml/err_check.log
#        ansible ${var_01} -m shell   -a  "ss -lntup| egrep  \"8080|42344|6443|\"" 
#        ansible   ${var_01} -m shell   -a  "ss -lntup| egrep  \"8080|42344|6443|\"" >>/root/K8s/Cluster_shell_yaml/err_check.log
# }
