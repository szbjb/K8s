...
master_ip=$(grep  master_hosts_  /root/K8s/ip_2.txt|awk -F  "[=]"   '{print  $NF}')
Local_ip=$(echo  $IP)
...
mkdir -pv $ETCD_SSL
scp ${master_hosts_}:/root/K8s/Software_package/etcd-v*-linux-amd64.tar.gz /root/K8s/Software_package/
...
tar -xvzf /root/K8s/Software_package/etcd-v*-linux-amd64.tar.gz  -C /root/K8s/Software_package/
\cp /root/K8s/Software_package/etcd-v*-linux-amd64/etcd* /usr/local/bin/
. /etc/etcd/etcd.conf
...
sed -i "/ETCD_LISTEN_PEER_URLS/{s/$master_ip/$IP/g}"  /etc/etcd/etcd.conf
...
rm -rf /root/K8s/Software_package/etcd-v*-linux-amd64*
...
case $shibieip in
        ...
        slav*)
         echo '1' 
         ssh ${master_hosts_} "test -f  /usr/lib/systemd/system/etcd.service"
         var_1=$(echo $?)
         u=0
         while  [ $var_1 -ne 0 ]
                           do
