...
mkdir -pv $KUBE_CONF
tar -xvzf /root/K8s/Software_package/flannel-v*-linux-amd64.tar.gz  -C  /root/K8s/Software_package/
cd  /root/K8s/Software_package/
mv {flanneld,mk-docker-opts.sh} /usr/local/bin/

yy_01=$(echo "$var_01"|sed  "s/,$//g")
oo_02=$(echo '$var_02')
#etcd old版本
...
etcdctl   --cacert=/etc/etcd/ssl/ca.pem   --cert=/etc/etcd/ssl/server.pem  --key=/etc/etcd/ssl/server-key.pem  --endpoints="https://${master_ip}:2379,${yy_01}"  endpoint   health  --write-out=table
etcdctl   --cacert=/etc/etcd/ssl/ca.pem   --cert=/etc/etcd/ssl/server.pem  --key=/etc/etcd/ssl/server-key.pem  --endpoints="https://${master_ip}:2379,${yy_01}"  endpoint   status  --write-out=table
...
--endpoints="http://${IP}:2379,$(echo $note |xargs -n 1  |sed  's/^/https:\/\/&/g'|sed 's/$/&:2379,/g' |xargs |sed '$s/,$//'| sed 's/ //g')" \
set /coreos.com/network/config  '{ "Network": "172.17.0.0/16", "Backend": {"Type": "vxlan"}}'
...
# etcdctl \
# --ca-file=ca.pem --cert-file=server.pem --key-file=server-key.pem \
# --endpoints="http://10.10.8.165:2379,https://10.10.8.160:2379,https://10.10.8.161:2379" \
# set /coreos.com/network/config  '{ "Network": "172.17.0.0/16", "Backend": {"Type": "vxlan"}}'
