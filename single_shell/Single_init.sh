...
tar -xzf /root/K8s/Software_package/etcd-v3.2.28-linux-amd64.tar.gz  -C /root/K8s/Software_package/
\cp -p /root/K8s/Software_package/etcd-v3.2.28-linux-amd64/etc* /usr/local/bin/
...
# ######
# sleep 1
# /usr/local/bin/kubectl   label node ${IP}  node-role.kubernetes.io/node='master'
# /usr/local/bin/kubectl   get node
# #修复授权
...
#安装helm
sh  /root/K8s/helm/install_helm.sh
sleep 5
#启用单机版nfs
